...
COMMON_FLAGS := -Wall -Wextra -Werror -I${COMMON_INCLUDES}
...
HOST_FLAGS := ${COMMON_FLAGS} `dpu-pkg-config --cflags --libs dpu` -DNR_TASKLETS=${NR_TASKLETS} -DNR_DPUS=${NR_DPUS} -DDPU_ENCRYPT_BINARY='"${DPU_ENCRYPT_TARGET}"' -DDPU_DECRYPT_BINARY='"${DPU_DECRYPT_TARGET}"' ${EXPERIMENT}
DPU_FLAGS := ${COMMON_FLAGS} -DNR_TASKLETS=${NR_TASKLETS} ${EXPERIMENT}
