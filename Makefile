...
COMMON_FLAGS := -Wall -Wextra -Werror -I${COMMON_DIR} -IPIM-common/common/include
...
HOST_FLAGS := ${COMMON_FLAGS} -I${HOST_DIR} -IPIM-common/host/include `dpu-pkg-config --cflags --libs dpu` -DNR_TASKLETS=${NR_TASKLETS} -DNR_DPUS=${NR_DPUS} -DDPU_ENCRYPT_BINARY='"${DPU_ENCRYPT_TARGET}"' -DDPU_DECRYPT_BINARY='"${DPU_DECRYPT_TARGET}"' ${EXPERIMENT}
