DPU_DIR := dpu
HOST_DIR := host
COMMON_DIR := common
BUILDDIR ?= build
NR_TASKLETS ?= 14
EXPERIMENTDIR ?= experiment

define conf_filename
	${BUILDDIR}/.NR_TASKLETS_$(2).conf
endef

CONF := $(call conf_filename,${NR_TASKLETS})

HOST_TARGET := ${BUILDDIR}/pimcrypto
DPU_ENCRYPT_TARGET := ${BUILDDIR}/dpu_encrypt
DPU_DECRYPT_TARGET := ${BUILDDIR}/dpu_decrypt

HOST_SOURCES := $(wildcard ${HOST_DIR}/*.c)
DPU_SOURCES := $(wildcard ${DPU_DIR}/*.c)
COMMON_SOURCES := $(wildcard ${COMMON_DIR}/*.c)

.PHONY: all clean experiment

__dirs := $(shell mkdir -p ${BUILDDIR})

COMMON_FLAGS := -Wall -Wextra -Werror -I${COMMON_DIR} -I${CURDIR}

ifdef EXPERIMENT
	COMMON_FLAGS += -O3
else
	COMMON_FLAGS += -g -O0
endif

HOST_FLAGS := ${COMMON_FLAGS} -I${HOST_DIR} `dpu-pkg-config --cflags --libs dpu` -DNR_TASKLETS=${NR_TASKLETS} -DDPU_ENCRYPT_BINARY='"${DPU_ENCRYPT_TARGET}"' -DDPU_DECRYPT_BINARY='"${DPU_DECRYPT_TARGET}"' ${EXPERIMENT}
DPU_FLAGS := ${COMMON_FLAGS} -I${DPU_DIR} -DNR_TASKLETS=${NR_TASKLETS} ${EXPERIMENT}

all: ${HOST_TARGET} ${DPU_ENCRYPT_TARGET} ${DPU_DECRYPT_TARGET}

${CONF}:
	$(RM) $(call conf_filename,*,*)
	touch ${CONF}

${HOST_TARGET}: ${HOST_SOURCES} ${COMMON_SOURCES} ${COMMON_INCLUDES} ${CONF}
	$(CC) -o $@ ${HOST_SOURCES} ${COMMON_SOURCES} ${HOST_FLAGS}

${DPU_ENCRYPT_TARGET}: ${DPU_SOURCES} ${COMMON_SOURCES} ${COMMON_INCLUDES} ${CONF}
	dpu-upmem-dpurte-clang ${DPU_FLAGS} -o $@ ${DPU_SOURCES} ${COMMON_SOURCES}

${DPU_DECRYPT_TARGET}: ${DPU_SOURCES} ${COMMON_SOURCES} ${COMMON_INCLUDES} ${CONF}
	dpu-upmem-dpurte-clang ${DPU_FLAGS} -DDECRYPT -o $@ ${DPU_SOURCES} ${COMMON_SOURCES}

experiment:
	$(RM) -r $(EXPERIMENTDIR)
	EXPERIMENT="-DEXPERIMENT" BUILDDIR=${EXPERIMENTDIR} NR_TASKLETS=${NR_TASKLETS} $(MAKE)

ctags: ${HOST_SOURCES} ${DPU_SOURCES} ${COMMON_SOURCES} ${COMMON_INCLUDES}
	ctags ${HOST_SOURCES} ${DPU_SOURCES} ${COMMON_SOURCES} $(wildcard ${COMMON_INCLUDES}/*.h)

clean:
	$(RM) -r $(BUILDDIR)

